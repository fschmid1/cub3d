...
/*   Updated: 2023/03/27 20:41:32 by pgorner          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_m *m, int x, int start, int end)
{
	while (start < end)
	{
		draw_pixel(m->map->img, start, x, m->map->color);
		start++;
	}
}
void	draw_ceiling(t_m *m, int x, int start, int end)
{
	...
	{
		if (i < start)
			draw_pixel(m->map->img, x, i, m->map->colorc);
		else if (i < end)
			draw_pixel(m->map->img, x, i, m->map->colorf);
		i++;
	}
}
